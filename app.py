...
# ---------------------------
# Sidebar: selecao de user
# ---------------------------
st.sidebar.header("User Explorer")
uid_input = st.sidebar.text_input("Digite user_id ou deixe vazio para amostra")
...
with tab_overview:
    ...
    sessions_count = df["session_id"].nunique()
    leads = df[df["event"].isin(["form_submit","purchase"])]["user_id"].nunique()
    ...
    col4.metric("Revenue", f"CA$ {revenue:,.2f}")
    col5.metric("CPL", f"CA$ {cpl:,.2f}")

    st.markdown("### Revenue & Sessions per Day")
    daily = df.groupby("date").agg(revenue=("revenue","sum"), sessions=("session_id","nunique")).reset_index()
    fig = px.line(daily, x="date", y=["revenue","sessions"], labels={"value":"Count","date":"Date"})
    st.plotly_chart(fig, use_container_width=True)

    st.markdown("### Sessions per Channel")
    channel_summary = df.groupby("channel").agg(sessions=("session_id","nunique"), revenue=("revenue","sum")).reset_index()
...
with tab_journey:
    ...
    # Criar Sankey: source -> page -> outcome
    first_events = df.sort_values("date").groupby("user_id").first().reset_index()
    last_events = df.sort_values("date").groupby("user_id").last().reset_index()

    sources = first_events["utm_source"].fillna("direct")
    pages = first_events["page"]
    outcome = df.groupby("user_id").last()["event"].apply(lambda x: "Converted" if x=="purchase" else "Not Converted")
    ...
    routes = df.groupby("user_id").apply(lambda g: " → ".join(g.sort_values("date")["page"].tolist())).reset_index(name="route")
...
with tab_user:
    ...
    uid = uid_input.strip()
    if uid=="":
        uid = np.random.choice(df["user_id"].unique())
...
with tab_debug:
    ...
    missing_utm = df[df["utm_source"].isna() | (df["utm_source"]=="")]
    st.write("Events missing UTM/source:", len(missing_utm))
    st.dataframe(missing_utm.head(50))

    sessions_with_no_purchase = df.groupby("session_id").filter(lambda x: not (x["event"]=="purchase").any())
    st.write("Sessions without purchase (sample):", sessions_with_no_purchase["session_id"].nunique())
    st.dataframe(sessions_with_no_purchase.head(50))
